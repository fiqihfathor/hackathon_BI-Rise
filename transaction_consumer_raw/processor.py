from db.models import Transaction, User, Device, IPAddress, Account, Merchant
...
def dict_to_transaction(event, user=None, device=None, account=None, merchant=None):
    ...
    def to_bool(val):
        ...
        if isinstance(val, str):
            return val.lower() == 'true'
        return False
    # transaction_id = UUID dari event['transaction_id'] (PK)
    # source_transaction_id = string eksternal dari event['source_transaction_id']
    return Transaction(
        id=event.get('transaction_id'),  # UUID, PK
        transaction_id=event.get('source_transaction_id'),  # string eksternal
        bank_id=event.get('bank_id'),
